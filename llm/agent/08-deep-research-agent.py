...
if __name__ == "__main__":
  for chunk in agent.stream(
    {"messages": [ HumanMessage( content= "What is the best way to learn about AI?")]},
    stream_mode="updates",
  ):
    for node, update in chunk.items():
      if not update or not (messages := update.get("messages")):
        continue
      mgs_list = messages.value if isinstance(messages, Overwrite) else messages
      for message in mgs_list:
